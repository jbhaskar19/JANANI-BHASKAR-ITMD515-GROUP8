...
public class LoginDAO {
	...
	private void fixQuery(Login login) {
		...
		String customerReset="from Customer where username = :username";
		String dealerReset="from Dealer where username = :username";
		String adminReset="from Admin where username = :username";
		if(login.getUserRole().equalsIgnoreCase("customer")) {
			QUERYSTRING=	customerUser;
			RESETQUERY=customerReset;
	...
	public Object validateUser(Login validateUser) {
		// TODO Auto-generated method stub
		AnnotationConfiguration config = new AnnotationConfiguration();
		  SessionFactory factory= config.configure().buildSessionFactory();
		  Session session = factory.openSession();
		  session.beginTransaction();
		  fixQuery(validateUser);
		  System.out.println(RESETQUERY); 
		  Query query = session.createQuery(RESETQUERY);

		  query.setString("username", validateUser.getUsername());
